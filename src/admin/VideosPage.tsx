...
export default function VideosPage() {
  ...
  const filteredVideos =
    filterFormat === "all"
      ? data.videos
      : data.videos.filter((v) => v.formatId === filterFormat);

  const videosByFormat = formats.map((f) => ({
    format: f,
    videos: data.videos
  ...
  const handleDragStart = (formatId: string, index: number) => {
    setDragState({ formatId, fromIndex: index });
  };

  const handleDragOver = (e: React.DragEvent, formatId: string, index: number) => {
    e.preventDefault();
    if (!dragState || dragState.formatId !== formatId) return;
    if (dragState.fromIndex === index) return;
  };

  const handleDrop = async (formatId: string, toIndex: number) => {
    if (!dragState || dragState.formatId !== formatId) return;
    const formatVideos = data.videos
      .filter((v) => v.formatId === formatId)
      .sort((a, b) => a.sortOrder - b.sortOrder);
  ...
  return (
    <div>
      <div className="mb-6 flex items-center justify-between">
        <h2 className="font-display text-xl font-black">ВИДЕО</h2>
        <button
      ...
      <div className="mb-6 flex flex-wrap gap-2">
        ...
        >
          Все ({data.videos.length})
        </button>
        {formats.map((f) => {
          const count = data.videos.filter((v) => v.formatId === f.id).length;
          return (
            <button
              key={f.id}
      ...
      {filterFormat === "all" ? (
        // Показ по форматам с drag-and-drop
        <div className="space-y-8">
          {videosByFormat.map(({ format, videos }) => (
            <div key={format.id}>
              ...
              {videos.length === 0 ? (
                <p className="font-mono text-xs text-coal-600">Нет видео</p>
              ) : (
                <div className="space-y-2">
                  {videos.map((video, index) => (
                    <VideoRow
                      ...
                      onDrop={handleDrop}
                      isDragging={dragState?.formatId === format.id && dragState.fromIndex === index}
                    />
                  ))}
                </div>
              )}
        ...
        <div className="space-y-2">
          {filteredVideos
            .sort((a, b) => a.sortOrder - b.sortOrder)
            .map((video, index) => {
              const format = formats.find((f) => f.id === video.formatId);
              return format ? (
                <VideoRow
                  key={video.id}
                  video={video}
                  ...
                  onTogglePublish={handleTogglePublish}
                  onDelete={handleDelete}
                  onDragStart={(formatId, i) => handleDragStart(formatId, i)}
                  onDragOver={(e, formatId, i) => handleDragOver(e, formatId, i)}
                  onDrop={(formatId, i) => handleDrop(formatId, i)}
                  isDragging={false}
                />
              ) : null;
            })}
...
interface VideoRowProps {
  ...
  onDragOver: (e: React.DragEvent, formatId: string, index: number) => void;
...
function VideoRow({
  ...
  return (
    <div
      draggable
      onDragStart={() => onDragStart(video.formatId, index)}
      onDragOver={(e) => onDragOver(e, video.formatId, index)}
      onDrop={() => onDrop(video.formatId, index)}
      className={`flex items-center gap-3 border border-line bg-coal-900 p-3 transition-all ${
        isDragging ? "opacity-50" : ""
      } hover:border-ember/40 cursor-grab active:cursor-grabbing`}
    >
      {/* Номер */}
      <span className="w-6 shrink-0 font-mono text-[10px] text-bone-dim">
        {String(index + 1).padStart(2, "0")}
      </span>

      {/* Постер */}
      <div className="h-10 w-16 shrink-0 overflow-hidden bg-coal-800">
      ...
      <div className="flex shrink-0 items-center gap-1.5">
        <button
          ...
          className={`border px-2 py-1 font-mono text-[9px] uppercase tracking-wider ${
            video.published
              ? "border-ember/50 text-ember hover:bg-ember hover:text-coal-950"
              : "border-coal-600 text-bone-dim hover:border-ember hover:text-ember"
          }`}
        >
          {video.published ? "Скрыть" : "Показать"}
        </button>
        <button
          onClick={() => onEdit(video)}
          className="border border-coal-600 px-2 py-1 font-mono text-[9px] uppercase tracking-wider text-bone-dim hover:border-bone-dim hover:text-bone"
        >
          Изм.
        </button>
        <button
          onClick={() => onDelete(video)}
          className="border border-coal-600 px-2 py-1 font-mono text-[9px] uppercase tracking-wider text-bone-dim hover:border-signal hover:text-signal"
        >
          ✕
        </button>
      </div>
